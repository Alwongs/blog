<li 
    ...
    @endif
>
    <a class="product-list-item__title" href="{{ route('products.show', $product->id) }}">
        &bull; {{$product->title}}
    </a>
                ...
                <li>
                    <form action="{{ route('products.update', $product) }}" method="POST">
                        ...
                        @if($product->status == App\Enum\Status::ACTIVE)
                            ...
                            <button class="action-list-disable" type="submit">{{ App\Enum\Status::STATUSES[App\Enum\Status::DISABLE] }}</button>
                        @else
                            <input type="hidden" name="status" value="{{ App\Enum\Status::ACTIVE }}" />
                            <button class="action-list-active" type="submit">{{ App\Enum\Status::STATUSES[App\Enum\Status::ACTIVE] }}</button>
                        @endif
                    </form>                    
                </li>
            </ul> 
